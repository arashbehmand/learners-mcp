...
from learners_mcp.study.streak import (
    compute_streak,
...
def test_streak_counts_consecutive_days(tmp_path):
    ...
    db.update_phase_data(
        sid, "explain", {"response": "r", "updated_at": _stamp(date(2026, 4, 17))}
    )
    db.update_phase_data(
        sid, "question", {"response": "r", "updated_at": _stamp(date(2026, 4, 18))}
    )
...
def test_streak_breaks_on_gap(tmp_path):
    ...
    db.update_phase_data(
        sid, "preview", {"response": "r", "updated_at": _stamp(date(2026, 4, 17))}
    )
    db.update_phase_data(
        sid, "explain", {"response": "r", "updated_at": _stamp(date(2026, 4, 19))}
    )
...
def test_weekly_report_shape(tmp_path):
    ...
    sid = db.create_section(mid, "A", "body", 1)
    db.update_phase_data(
        sid, "preview", {"response": "r", "updated_at": _stamp(date(2026, 4, 18))}
    )
    db.create_flashcard(
        mid, sid, "Q", "A", created_at=datetime(2026, 4, 18, 12, 0, tzinfo=timezone.utc)
    )
